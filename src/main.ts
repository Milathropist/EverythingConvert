...
(window as Window & { __convertAppStarted?: boolean }).__convertAppStarted = true;

function setupInteractiveBackground () {
  const canvas = document.querySelector("#interactive-background-canvas");
  ...
  const config = {
    dotColor: "rgba(144, 238, 144, 0.78)",
    lineColor: [144, 238, 144],
    dotMinSize: 1,
    dotMaxSize: 3,
    dotSpeed: 0.35,
    dotDensityFactor: 11000,
  ...
  const initDots = () => {
    dots = [];
    const dotCount = Math.max(28, Math.floor((window.innerWidth * window.innerHeight) / config.dotDensityFactor));
    for (let i = 0; i < dotCount; i++) {
      const size = Math.random() * (config.dotMaxSize - config.dotMinSize) + config.dotMinSize;
      dots.push({
        x: Math.random() * window.innerWidth,
        y: Math.random() * window.innerHeight,
        dx: (Math.random() - 0.5) * config.dotSpeed,
        dy: (Math.random() - 0.5) * config.dotSpeed,
        size
      });
    }
  };
  ...
  const drawFrame = () => {
    ...
    for (const dot of dots) {
      ...
      if (dot.x + dot.size > window.innerWidth || dot.x - dot.size < 0) dot.dx = -dot.dx;
      if (dot.y + dot.size > window.innerHeight || dot.y - dot.size < 0) dot.dy = -dot.dy;

      drawDot(dot);

      if (mouse.x === null || mouse.y === null) continue;

      const distance = Math.hypot(mouse.x - dot.x, mouse.y - dot.y);
      if (distance >= config.connectionRadius) continue;

      const alpha = 1 - distance / config.connectionRadius;
      ctx.strokeStyle = `rgba(${config.lineColor.join(", ")}, ${alpha})`;
      ctx.lineWidth = 1;
      ctx.beginPath();
...
function loadingMarkup (title: string, detail?: string): string {
  return `<div class="loading-shell" role="status" aria-live="polite">
...
function showLoadingPopup (title: string, detail?: string): void {
  window.showPopup(loadingMarkup(title, detail));
}

function showLoadingError (err: unknown): void {
  window.showPopup(
    ...
    return await Promise.race([
      ...
      new Promise<T>((_, reject) => {
        timeoutId = window.setTimeout(
          () => reject(new Error(`${label} did not finish loading in ${Math.round(ms / 1000)} seconds.`)),
          ms
        );
      })
    ]);
...
async function buildOptionList () {
  ...
  showLoadingPopup("Loading tools...", "Preparing converter assets");
  ...
  for (const [handlerIndex, handler] of handlers.entries()) {
    if (!window.supportedFormatCache.has(handler.name)) {
      ...
      try {
        await withTimeout(Promise.resolve(handler.init()), HANDLER_INIT_TIMEOUT_MS, handler.name);
      } catch (err) {
        console.warn(`Handler "${handler.name}" did not finish initializing: ${getErrorMessage(err)}`);
        continue;
      }
  ...
  if (allOptions.length === 0) {
    throw new Error("No conversion tools finished loading. Check that cache.json and copied assets are available.");
  }
...
(async () => {
  try {
    showLoadingPopup("Loading tools...", "Reading format cache");
    const cacheJSON = await fetch("cache.json").then(r => {
      if (!r.ok) throw new Error(`cache.json returned ${r.status}`);
      return r.json();
    });
  ...
  try {
    ...
    console.error("Failed to build initial format list.", err);
...
ui.modeToggleButton.addEventListener("click", () => {
  ...
  if (simpleMode) {
    ...
    document.body.style.setProperty("--highlight-color", "#90ee90");
  } else {
    ui.modeToggleButton.textContent = "Simple mode";
    document.body.style.setProperty("--highlight-color", "#eea2d6");
  }
  buildOptionList().catch(showLoadingError);
});
...
async function attemptConvertPath (files: FileData[], path: ConvertPathNode[]) {
  ...
  ui.popupBox.innerHTML = loadingMarkup("Finding conversion route...", `Trying ${pathString}...`);
  ...
  for (let i = 0; i < path.length - 1; i ++) {
    ...
    try {
      ...
      ui.popupBox.innerHTML = loadingMarkup("Finding conversion route...", "Looking for a valid path...");
...
ui.convertButton.onclick = async function () {
  ...
  try {
    ...
    showLoadingPopup("Finding conversion route...", "Looking for a valid path...");
