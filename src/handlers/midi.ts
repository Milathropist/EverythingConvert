...
function loadFluidSynth(): Promise<{ JSSynth: any; sfontBin: ArrayBuffer }> {
  if (!midiInitPromise) {
    midiInitPromise = (async () => {
      ...
      let fluidSrc = await fetch(appAssetUrl("wasm/libfluidsynth-2.4.6.js")).then(r => r.text());
      ...
      await loadScript(appAssetUrl("wasm/js-synthesizer.js"));
      ...
      const sfontBin = await fetch(appAssetUrl("wasm/TimGM6mb.sf2")).then(r => r.arrayBuffer());
